...
export function setRedrawCallback(fn) { _redrawCallback = fn; }
            ...
            if (title) {
                ...
                return wrapText(ctx, title, 256, settings['titleYOffset'], titleMaxWidth, settings['titleLineHeight'])
                    .then(numberOfTitleLines => {
                        ...
                        return wrapText(ctx, description, settings['descriptionXOffset'] || 256, settings['descriptionYOffset'] + ((numberOfTitleLines - 1) * settings["titleLineHeight"]), descriptionMaxWidth, settings['descriptionLineHeight'], true);
                    })
                    .then(() => {
...
async function wrapText(context, text, x, y, maxWidth, lineHeight = 16, isDescription = false) {
    ...
    for (let index = 0; index < finalLines.length; index++) {
        ...
        inRulesSection = await writeCharacters(context, line, x, (y + (index * lineHeight)), inRulesSection, isDescription);
    }
...
function getLines(ctx, text, maxWidth) {
    ...
    for (let i = 1; i < words.length; i++) {
        ...
        currentLineWidth = ctx.measureText(currentLineNoTags).width + sumImgTagWidths(currentLine, fontSize);

        // Calculate width for the word being added
        const wordNoTags = word.replaceAll(/(<([^>]+)>)/gi, "");
        wordWidth = ctx.measureText(wordNoTags).width + sumImgTagWidths(word, fontSize);
...
function writeCharacters(ctx, str, x, y, inRulesSection, isDescription = false) {
            ...
            if (ch === "<") {
                ...
                if (endOfTag !== -1) {
                    ...
                    if (tagContent.toLowerCase().startsWith('img')) {
                        ...
                        const modifierMatch = tagContent.substring(3).match(/^(.+?)([+-]\d+)$/);
                        if (modifierMatch) {
                        ...
                        if (imageUrl) {
                            ...
                            const refMultiplier = inlineImageUrlMap[keyword] ? (settings['refIconScale'] || 2) : 1;
